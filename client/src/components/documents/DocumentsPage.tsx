...
import { fetchDocuments, fetchDocumentById, createDocument, deleteDocument } from "@/lib/documents-client";
import dynamic from "next/dynamic";
import { PageLayout } from "@/components/layout/PageLayout";
import { DocumentsDataTable } from "./DocumentsDataTable";

// Dynamically import the document viewer
const DocumentViewer = dynamic(() => import("./DocumentViewComponent"), {
  ssr: false,
});
...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    const handleDocumentInUrl = async () => {
      ...
      // Get URL parameters using React Router's location
      ...
      // Check for document ID in path parameters
      ...
      try {
        ...
        if (doc) {
          ...
          if (shouldViewDocument) {
            ...
            const newUrl = `${location.pathname}?docId=${documentId}`;
  ...
  const handleViewDocument = useCallback((doc: any) => {
    ...
    const newUrl = `${location.pathname}?docId=${doc.id}`;
  ...
  const handleDocumentDeleted = async (id: string) => {
    try {
      ...
      setError(`Error deleting document: ${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  return (
    <PageLayout
      title="Documents"
      subtitle="Manage your documents and policies"
      actions={
